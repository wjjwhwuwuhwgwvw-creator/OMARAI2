...
import { splitFile, splitFileFromUrl, needsSplitting, getJoinInstructions, cleanupParts, MAX_WHATSAPP_SIZE, TEMP_DIR, formatBytes as formatSplitBytes } from './src/utils/file-splitter.js';
                ...
                try {
                    const sections = [{
                        ...
                        rows: cleanResults.map((app, idx) => ({
                            ...
                            description: app.isMod ? `مهكرة - ${app.source || 'AN1'}` : (app.developer ? `المطور: ${app.developer}` : '')
                        }))
                    }];
...
async function handleAppDownload(sock, remoteJid, userId, senderPhone, msg, appId, appTitle, session, isAdmin = false, selectedApp = null) {
    ...
    console.log(`✅ تختار: ${appTitle} (${isModDownload ? 'MOD: ' + selectedApp.url : appId})`);
    ...
    if (isModDownload) {
        ...
        await sock.sendMessage(remoteJid, { react: { text: '🔓', key: msg.key } });
        await sendBotMessage(sock, remoteJid, { 
            text: `🔓 *تحميل نسخة مهكرة*\n\n📱 ${appTitle}\n📦 المصدر: ${modSource}\n\n⏳ جاري جلب رابط التحميل...${POWERED_BY}` 
        }, msg);

        try {
            const downloadEndpoint = '/an1-download';
            console.log(`📥 [MOD] Using endpoint: ${downloadEndpoint} for ${selectedApp.url}`);
            ...
            if (response.data?.download_url) {
                ...
                // Check file size first with HEAD request (for display only)
                let estimatedSize = 0;
                try {
                ...
                await sendBotMessage(sock, remoteJid, { 
                    text: `⏬ جاري تحميل الملف${estimatedSize > 0 ? ` (${formatFileSize(estimatedSize)})` : ''}...${POWERED_BY}` 
                }, msg);
                await sock.sendMessage(remoteJid, { react: { text: '⏬', key: msg.key } });
                ...
                if (downloadResult.needsSplit) {
                    ...
                    try {
                        for (const part of parts) {
                            ...
                            const partFileName = `${sanitizedName} (مهكرة).apk.part${String(part.partNumber).padStart(3, '0')}`;
                            ...
                            if (part.partNumber === 1) {
                                caption += `\n${getJoinInstructions(`${sanitizedName} (مهكرة).apk`, parts.length)}`;
                            }
                            caption += POWERED_BY;
